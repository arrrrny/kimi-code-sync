...
| `KIMI_CODE_EXPERIMENTAL_COMPACTION_HANDOFF` | Experimental compaction handoff: before an automatic context compaction, a structured handoff document (session gist, current state, open threads with next actions, gotchas, files touched, open questions) is saved beside the session records; the post-compaction summary and the TUI completion message point at it; `KIMI_CODE_EXPERIMENTAL_FLAG=1` also enables it | Truthy: `1`/`true`/`yes`/`on`; falsy: `0`/`false`/`no`/`off` |
