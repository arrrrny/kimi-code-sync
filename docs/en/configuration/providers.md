...
**Credential priority**: the key selected by `active_api_key_id` in `api_keys` > `api_key` direct field > `[providers.<name>.env]` sub-table key > if all are absent, startup fails with an error. The CLI does not fall back to shell environment variables for credentials. See [Config overrides: provider credentials](./overrides.md#provider-credentials).
