...
2. **Conversation context** — if the current session has just run `/skill:speckit-bug-assess` or `/skill:speckit-bug-fix`, the slug it reported is the working slug. Reuse it without re-prompting. Confirm it by checking that `.specify/bugs/<slug>/fix.md` exists; if it does not, **stop** and instruct the user to run `/skill:speckit-bug-fix` for that slug first. Do NOT fall through to disk discovery when a context-named slug's fix.md is missing.
